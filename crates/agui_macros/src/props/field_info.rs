...
use super::util::{
    apply_subsections, expr_to_lit_string, expr_to_single_string, ident_to_type,
    path_to_single_string, strip_raw_ident_prefix,
};
        ...
        match expr {
            ...
            syn::Expr::Path(path) => {
                ...
                match name.as_str() {
                    "default" => {
                        self.default =
                            Some(syn::parse2(quote!(::core::default::Default::default())).unwrap());
                        Ok(())
                    }
                    _ => Err(Error::new_spanned(
                        &path,
