...
use super::{
    CrossAxisAlignment, FlexChildParams, MainAxisAlignment, MainAxisSize, VerticalDirection,
};
...
impl RenderFlex {
    pub fn update_direction(&mut self, ctx: &mut RenderObjectUpdateContext, direction: Axis) {
    ...
    pub fn update_main_axis_size(
        &mut self,
        ctx: &mut RenderObjectUpdateContext,
        main_axis_size: MainAxisSize,
    ) {
        if self.main_axis_size == main_axis_size {
            return;
        }
    ...
    pub fn update_main_axis_alignment(
        &mut self,
        ctx: &mut RenderObjectUpdateContext,
        main_axis_alignment: MainAxisAlignment,
    ) {
        if self.main_axis_alignment == main_axis_alignment {
            return;
        }
    ...
    pub fn update_cross_axis_alignment(
        &mut self,
        ctx: &mut RenderObjectUpdateContext,
        cross_axis_alignment: CrossAxisAlignment,
    ) {
        if self.cross_axis_alignment == cross_axis_alignment {
            return;
        }
    ...
    pub fn update_vertical_direction(
        &mut self,
        ctx: &mut RenderObjectUpdateContext,
        vertical_direction: VerticalDirection,
    ) {
        if self.vertical_direction == vertical_direction {
            return;
        }
    ...
    pub fn update_children_params(
        &mut self,
        ctx: &mut RenderObjectUpdateContext,
        children_params: Vec<FlexChildParams>,
    ) {
        if self.children_params == children_params {
            return;
        }

        self.children_params = children_params;
        ctx.mark_needs_layout();
    }
}

impl RenderObjectImpl for RenderFlex {
    fn intrinsic_size(
        &self,
        ...
        if self.direction == dimension.axis() {
            ...
            for child in ctx.iter_children() {
                let flex = self.children_params[child.index()].flex;
...
impl RenderFlex {
    fn compute_sizes(
        &self,
        ctx: &mut RenderObjectLayoutContext,
        ...
        while let Some(mut child) = children.next() {
            ...
            if flex > 0.0 {
                total_flex += flex;
                last_flexible_child = Some(child.render_object_id());
            } else {
        ...
        if total_flex > 0.0 {
            ...
            while let Some(mut child) = children.next() {
                let flex = self.children_params[child.index()].flex;
                let fit = self.children_params[child.index()].fit;

                if flex > 0.0 {
                    let max_child_extent = if can_flex {
                        if Some(child.render_object_id()) == last_flexible_child {
