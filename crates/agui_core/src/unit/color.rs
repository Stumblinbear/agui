...
impl Color {
    ...
    pub const fn from_rgb([red, green, blue]: [f32; 3]) -> Color {
        Self::from_rgba([red, green, blue, 1.0])
    }

    pub const fn from_rgba([red, green, blue, alpha]: [f32; 4]) -> Color {
        Color {
            red,
            green,
            blue,
...
impl PartialEq for Color {
    fn eq(&self, other: &Self) -> bool {
        (self.red - other.red).abs() < EQ_MARGIN_OF_ERROR
            && (self.green - other.green).abs() < EQ_MARGIN_OF_ERROR
            && (self.blue - other.blue).abs() < EQ_MARGIN_OF_ERROR
            && (self.alpha - other.alpha).abs() < EQ_MARGIN_OF_ERROR
    }
}
...
impl From<Color> for u32 {
    fn from(color: Color) -> Self {
        let val = color.as_rgba();

        ((val[0] * 255.0) as Self) << 24
            | ((val[1] * 255.0) as Self) << 16
            | ((val[2] * 255.0) as Self) << 8
            | (val[3] * 255.0) as Self
    }
}
